...
const ShimmerCard = ({ isLargeRow }) => {
    ...
    return (
        <div
            className={`relative flex-shrink-0 rounded-lg sm:rounded-xl overflow-hidden ${theme === "dark" ? "bg-gray-800" : "bg-gray-300"} ${cardWidth}`}
            aria-hidden="true"
        >
            {/* Smooth, cinematic shimmer fill */}
            <div className="absolute inset-0 shimmer-cinematic rounded-lg sm:rounded-xl" />
